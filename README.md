...
### 2025-12-21 发布适配与错误修复
- 资源路径全面适配 `import.meta.env.BASE_URL`，同时兼容本地开发与 GitHub Pages 子路径部署
- 配置 `vite.config.ts` 的 `base: '/christmas-tree-with-photos/'` 与 `server.open`，本地自动打开正确路由
- 修复媒体播放 `AbortError`，为 `video.play()` 与 `audio.play()` 添加安全处理与静默捕获
- 修复 `index.html` 绝对路径引用，统一改为相对路径，避免子路径下加载失败
- 调试窗口缩小至 `200px`，减少遮挡
- 完善图片放大交互的淡入淡出过渡，手指松开后更自然
- 彩蛋效果从“升空烟花”改为“中心彩色纸屑炸开”，统一多彩配色与运动逻辑
- 顶部标题简化为 “Merry Christmas~”
- 文档新增致谢章节，明确本项目为在原仓库基础上的二次开发
